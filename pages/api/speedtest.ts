...
export const config = {
  api: {
    bodyParser: false, // REQUIRED for accurate upload speed test via streaming
  },
};
  ...
  if (req.method === 'POST') {
    ...
    try {
      await new Promise((resolve, reject) => {
        // No body parsing means we handle the raw stream
        req.on('data', () => {}); 
        req.on('end', resolve);
        req.on('error', reject);
      });
      ...
      console.error('Speed test upload error:', e);
