...
export default function MobileDashboard() {
  ...
  const loadDashboardStats = async () => {
    ...
    try {
      ...
      const cacheKey = `mobile-dashboard:stats:${orgId}:${eventKey}`;
  ...
  const loadRecentActivity = async () => {
    ...
    try {
      ...
      const activityCacheKey = `mobile-dashboard:recent-activity:${orgId}`;
      ...
      // Resolve CSV event scope for scoping activity and pit data
      ...
      const recent = activities.slice(0, 5).map(({ _sort, ...rest }) => rest);
                    ...
                    {loadingStats ? (
                      ...
                      <>
                        ...
                        <p className="text-xs text-primary font-medium">
                          {dashboardStats.totalMatches > 0 ? 'TBA schedule (this event)' : 'Sync event in Team Management'}
                        </p>
                      </>
                    )}
                  </div>
                    ...
                    {loadingStats ? (
                      ...
                      <>
                        ...
                        <p className="text-xs text-secondary font-medium">
                          {dashboardStats.teamsCount > 0 ? 'On event roster (TBA)' : 'No roster — sync TBA'}
                        </p>
                      </>
                    )}
                  </div>
